...
for i = 1:length(kcoords2)
    ...
    cluname = fullfile([basename '.clu.' num2str(i)]);
    fid=fopen(cluname,'w');
    fprintf(fid,'%.0f\n',tclu);
    fclose(fid);
    clear fid
...
for i = 1:length(kcoords2)
    ...
    if mod(i,4)==1; fprintf('\n'); end
    fprintf(['Saving .res file for group ', num2str(i),'. '])
    resname = fullfile([basename '.res.' num2str(i)]);
    fid=fopen(resname,'w');
    fprintf(fid,'%.0f\n',tspktimes);
    fclose(fid);
    clear fid
...
if   length(kcoords2)==1 && rez.ops.NchanTOT>16
    fprintf('Extracting Spikes for linear shanks\n\n')
    Process_ExtractLargeSpk(rez);
else

    %waveforms_all = Kilosort_ExtractWaveforms(rez);
    nChannels = Kilosort_ExtractWaveforms(rez);
    %nChannels = [10;10;10;10;10;10;4;8;8;8;8];
    clear rez;
    ...
    for i = 1:length(kcoords2)
        ...
        waveforms = LoadSpikeWaveforms([basename,'.spk.',num2str(i)],nChannels(i),nSamples);
        %waveforms = LoadSpikeWaveforms([basename,'.spk.',num2str(i)],8,40);
        ...
        fid=fopen([basename,'.fet.',num2str(i)],'w');
...
function nChannels = Kilosort_ExtractWaveforms(rez)
        ...
        for i = 1:length(kcoords2)
            ...
            %Opsn spk files
            fidWav{i} = fopen([basename,'.spk.',num2str(i)],'w');
            savedSpikes{i} = false(length(spikeTimes(ia{i})),1);
        ...
        for ibatch = 1:Nbatch
            ...
            for i = 1:length(kcoords2)
                ...
                temp = ismember(spikeTimes(ia{i}), [nSamples/2+1:size(dataRAW,1)-nSamples/2] + dat_offset);
                ...
                %some spikes may overlap from one iteration to the next...
                exclSpks = savedSpikes{i}(temp);
                
                waveforms = reshape(dataRAW(X,indicesTokeep{i})',size(indicesTokeep{i},1),nSamples,[]);
                waveforms = waveforms(:,:,~exclSpks);
                
                %let's keep track of which spikes have been saved.
                savedSpikes{i}(temp) = true;
                
                %Write waveforms
                fwrite(fidWav{i},waveforms(:),'int16');
        ...
        nChannels = zeros(length(kcoords2),1);
         ...
         for i = 1:length(kcoords2)
            fclose(fidWav{i});
            nChannels(i) = length(indicesTokeep{i});
         end
         keyboard
    end
end
